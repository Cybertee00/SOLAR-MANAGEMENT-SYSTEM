// Improved Excel parser with better header detection for merged cells and complex formats
// Specifically handles Concentrated Cabinet and other complex templates
...
/**
 * Extract cell value as string (handles merged cells)
 */
...
/**
 * Improved header row detection - handles merged cells and various formats
 */
function findHeaderRowImproved(worksheet, maxRows = 50) {
  // Strategy 1: Look for row with "ITEM" and CT numbers or inverter codes
  for (let rowNum = 5; rowNum <= maxRows; rowNum++) {
    ...
    for (let colNum = 1; colNum <= Math.min(worksheet.columnCount, 30); colNum++) {
      ...
      if (upperVal.match(/CT\d+/)) {
        hasCT = true;
      }
      if (upperVal.match(/C\d{3}/) && !upperVal.match(/CT/)) {
        hasInverter = true;
      }
      if (upperVal.includes('DESCRIPTION') || upperVal.includes('CHECK') || upperVal.includes('INSPECT')) {
        hasDescription = true;
      }
    }
    
    // If we found item + (CT or inverter or description), this is likely the header
    if (hasItem && (hasCT || hasInverter || hasDescription)) {
      return {
        rowNum,
        values: rowValues,
        columns: extractColumnsImproved(rowValues, worksheet, rowNum)
      };
    }
  }
  
  // Strategy 2: Look for row with multiple CT numbers (even without "ITEM")
  for (let rowNum = 5; rowNum <= maxRows; rowNum++) {
    const row = worksheet.getRow(rowNum);
    const rowValues = [];
    let ctCount = 0;
    ...
    for (let colNum = 1; colNum <= Math.min(worksheet.columnCount, 30); colNum++) {
      ...
      if (value.match(/CT\d+/i)) ctCount++;
      if (value.match(/C\d{3}/i) && !value.match(/CT/i)) inverterCount++;
    }
    
    // If we found 2+ CT numbers or inverters, this is likely the header
    if (ctCount >= 2 || inverterCount >= 2) {
      return {
        rowNum,
        values: rowValues,
        columns: extractColumnsImproved(rowValues, worksheet, rowNum)
...
/**
 * Improved column extraction with merged cell support
 */
function extractColumnsImproved(headerValues, worksheet, headerRowNum) {
  const columns = [];
...
/**
 * Parse worksheet with improved detection
 */
async function parseWorksheetImproved(worksheet, sheetName) {
  ...
  const headerInfo = findHeaderRowImproved(worksheet);
  if (!headerInfo) {
    console.log(`    ⚠️  No header row found (tried improved detection)`);
    return result;
  }
  ...
  // If no explicit item column, try to infer (usually first column with numbers)
  let inferredItemCol = itemCol;
  if (!inferredItemCol) {
    for (let colNum = 1; colNum <= Math.min(5, worksheet.columnCount); colNum++) {
      const testCell = worksheet.getRow(headerInfo.rowNum + 1).getCell(colNum);
  ...
  // If no explicit description column, try to infer (usually second column)
  let inferredDescCol = descCol;
  if (!inferredDescCol && inferredItemCol) {
    inferredDescCol = { index: inferredItemCol.index + 1, type: 'description' };
  }
  
  if (!inferredItemCol && !inferredDescCol) {
  ...
  const maxRows = Math.min(dataStartRow + 250, worksheet.rowCount);
  ...
  for (let rowNum = dataStartRow; rowNum <= maxRows; rowNum++) {
    ...
    const resultValue = resultCol ? getCellValue(row.getCell(resultCol.index), worksheet) : '';
...
/**
 * Parse Excel file with improved detection
 */
async function parseExcelFileImproved(filePath) {
  ...
  try {
    ...
    // Extract procedure and title from first sheet (check more rows)
    ...
    for (let rowNum = 1; rowNum <= 15; rowNum++) {
      ...
      for (let colNum = 1; colNum <= Math.min(10, firstSheet.columnCount); colNum++) {
        ...
        if (upperVal.includes('PROCEDURE') || upperVal.match(/PM-\d+/)) {
          if (!result.procedure) {
    ...
    for (const worksheet of workbook.worksheets) {
      const sheetData = await parseWorksheetImproved(worksheet, worksheet.name);
...
/**
 * Convert to checklist template (same as before but with improved data)
 */
function convertToChecklistTemplateImproved(parsedData) {
  if (!parsedData || parsedData.sheets.length === 0) {
    return null;
  }
  
  // Find sheet with most items
  const bestSheet = parsedData.sheets.reduce((best, sheet) => 
    sheet.items.length > (best.items.length || 0) ? sheet : best
  );
  
  if (bestSheet.items.length === 0) {
    return null;
  }
  ...
  bestSheet.items.forEach((item, idx) => {
...
function generateTemplateCode(fileName, procedure) {
  ...
  if (upperFileName.includes('CONCENTRATED CABINET')) assetType = 'CONC_CABINET';
  else if (upperFileName.includes('ENERGY METER')) assetType = 'ENERGY_METER';
  else if (upperFileName.includes('INVERTER')) assetType = 'INVERTER';
  else if (upperFileName.includes('STRING COMBINER')) assetType = 'STRING_COMBINER';
  else if (upperFileName.includes('VENTILATION')) assetType = 'VENTILATION';
  else if (upperFileName.includes('CCTV')) assetType = 'CCTV';
  else if (upperFileName.includes('SCADA')) assetType = 'SCADA';
  else if (upperFileName.includes('SAFETY') || upperFileName.includes('FIRE')) assetType = 'SAFETY_FIRE';
  else if (upperFileName.includes('MV') || upperFileName.includes('MEDIUM VOLTAGE')) assetType = 'MV_SWITCHGEAR';
  else if (upperFileName.includes('BTU') || upperFileName.includes('SUBSTATION')) assetType = 'SUBSTATION';
 ...
 * Main function
 */
async function parseAllTemplatesImproved() {
  ...
  console.log('='.repeat(80));
  console.log('IMPROVED EXCEL TEMPLATE PARSING');
  console.log('='.repeat(80));
  console.log(`\nParsing ${excelFiles.length} Excel templates with improved detection...\n`);
  ...
  for (const fileName of excelFiles) {
    ...
    const parsed = await parseExcelFileImproved(filePath);
    
    if (parsed) {
      parsedResults.push(parsed);
      
      const template = convertToChecklistTemplateImproved(parsed);
  ...
  const parsedOutputPath = path.join(outputDir, 'excel-parsed-results-improved.json');
  fs.writeFileSync(parsedOutputPath, JSON.stringify(parsedResults, null, 2), 'utf8');
  console.log(`\n✓ Parsed data saved to: ${parsedOutputPath}`);
  
  const templatesOutputPath = path.join(outputDir, 'excel-checklist-templates-improved.json');
  ...
  templates.forEach(template => {
    ...
    if (template.checklist_structure.metadata.has_ct_buildings) {
      console.log(`  ✓ Has CT Buildings (City Buildings/Inverter Buildings)`);
    }
    if (template.checklist_structure.metadata.has_inverters) {
      console.log(`  ✓ Has Inverters (C001, C002, etc.)`);
    }
  });
...
module.exports = { parseExcelFileImproved, parseAllTemplatesImproved, convertToChecklistTemplateImproved };
