...
async function debug() {
  const filePath = path.join(__dirname, '../server/templates/excel/Energy Meter_Checklist.xlsx');
  const workbook = new ExcelJS.Workbook();
  await workbook.xlsx.readFile(filePath);
  
  const sheet = workbook.worksheets[0];
  
  console.log('Energy Meter - Rows 11-20:');
  for (let rowNum = 11; rowNum <= 20; rowNum++) {
