...
// Run from server directory: node scripts/import-excel-templates.js
...
async function importTemplates() {
  try {
    ...
    // Load templates from project root
    const templatesPath = path.join(__dirname, '../../server-templates-final.json');
    ...
    for (const template of templates) {
      try {
        ...
        if (template.checklist_structure.metadata && template.checklist_structure.metadata.has_ct_buildings) {
          console.log(`  - CT Buildings: ${template.checklist_structure.metadata.ct_buildings.map(c => c.code).join(', ')}`);
        }
        if (template.checklist_structure.metadata && template.checklist_structure.metadata.has_inverters) {
    ...
    console.log('\n✓ Import complete!');
    console.log('\nRefresh your browser to see the new templates in the Checklist Templates page.');
